...
// HealthyWormCoin: optional setting to unlock wallet for block minting only;
...
// HealthyWormCoin: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // HealthyWormCoin: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                // HealthyWormCoin: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    else
                    {
                        ...
                        if (!GetBoolArg("-avatar")) // HealthyWormCoin: not avatar mode
                        {
...
// HealthyWormCoin: create coin stake transaction
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64 nSearchInterval, CTransaction& txNew)
{
    ...
    CBigNum bnTargetPeHealthyWormCoinDay;
    bnTargetPeHealthyWormCoinDay.SetCompact(nBits);
...
// HealthyWormCoin: check 'spent' consistency between wallet and txindex
// HealthyWormCoin: fix wallet spent state according to txindex
...
// HealthyWormCoin: disable transaction (only for coinstake)
